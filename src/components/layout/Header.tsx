  ...
  return (
    ...
    >
      ...
      <div className="hidden lg:block bg-[#111111] text-white text-xs py-1.5 text-center">
        <span className="text-[#fcd502] font-semibold">Salem&apos;s #1 Custom Printing</span>
        &nbsp;·&nbsp; Same-day dispatch for orders before 12 PM &nbsp;·&nbsp;
        <span className="text-[#01a2fb]">Free delivery above ₹999</span>
      </div>
      ...
      <div className="container-app">
        <div className="flex items-center gap-2 sm:gap-3 h-16">
          ...
          <button className="hidden lg:flex items-center gap-1.5 px-3 py-2 rounded-full border border-[#e5e7eb] hover:border-[#01a2fb] transition-colors text-sm text-[#444] hover:text-[#01a2fb] flex-shrink-0">
            <MapPin size={14} className="text-[#fa028e]" />
          ...
          <nav className="hidden lg:flex items-center gap-5 mx-2 flex-shrink-0 text-[15px] font-bold text-[#444]">
            ...
            <Link href="/about" className="hover:text-[#01a2fb] transition-colors">About Us</Link>
            <Link href="/products?category=bulk-printing" className="hover:text-[#01a2fb] transition-colors">Bulk Order</Link>
            <Link href="/contact" className="hover:text-[#01a2fb] transition-colors">Contact Us</Link>
          </nav>

          {/* Search bar */}
          <form onSubmit={handleSearch} className="flex-1 relative max-w-2xl min-w-0">
            <div className={`flex items-center border-2 rounded-full px-3 sm:px-4 py-1.5 sm:py-2 transition-all duration-300 ${isSearchFocused ? 'border-[#01a2fb] bg-white shadow-[0_4px_15px_rgba(0,174,239,0.15)] ring-4 ring-[#01a2fb]/10' : 'bg-white border-[#e5e7eb] shadow-sm hover:border-[#01a2fb]/50 hover:shadow-md'}`}>
              <Search size={16} className={`flex-shrink-0 hidden sm:block transition-colors ${isSearchFocused ? 'text-[#01a2fb]' : 'text-[#888]'}`} />
              <Search size={14} className={`flex-shrink-0 sm:hidden transition-colors ${isSearchFocused ? 'text-[#01a2fb]' : 'text-[#888]'}`} />
              <input
                ref={searchRef}
                type="text"
          ...
          <div className="flex items-center gap-1 flex-shrink-0">
            ...
            >
              ...
              {isMounted && itemCount > 0 && (
                <span className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-[#fa028e] text-white text-[12px] font-bold flex items-center justify-center leading-none animate-scaleIn">
