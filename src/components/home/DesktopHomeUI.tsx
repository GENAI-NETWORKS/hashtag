            ...
            {CATEGORIES.map((cat) => {
              return (
                <Link key={cat.id} href={`/products?category=${cat.slug}`}
                  ...
                  <span className="text-[13px] sm:text-xs font-semibold text-center leading-tight text-[#444] group-hover:text-[#111] transition-colors">{cat.name}</span>
                </Link>
              );
            })}
          </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-2">
                  <div className="flex -space-x-2">
                    {[1, 2, 3].map(i => <div key={i} className="w-8 h-8 rounded-full bg-[#e5e7eb] border-2 border-white flex items-center justify-center text-[12px] text-[#888] font-bold">≡ƒÿè</div>)}
                  </div>
